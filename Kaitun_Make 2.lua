...
Interface.BlurManager = blurEffect
    ...
    -- 🥊 Kiểm tra skill đặc biệt của Black Leg / Death Step
function CheckMeleeBurstMove(Child)
    if Child.Name == "Black Leg" or Child.Name == "Death Step" then
        local skillsUI = PlayerGui:FindFirstChild("Main") and PlayerGui.Main:FindFirstChild("Skills")
        if not skillsUI then return end

        local meleeUI = skillsUI:FindFirstChild(Child.Name)
        if meleeUI then
            local vSkill = meleeUI:FindFirstChild("V")
            if vSkill then
                ResearchMoves(vSkill)
            end
        end
    end
end

-- 🥋 Refresh danh sách Melee mastery
...
-- 🥊 Kiểm tra khi có Tool / Fruit mới
function MeleeCheck(Child)
    if not Child or typeof(Child) ~= "Instance" or not Child:IsA("Tool") then return end

    if Child.ToolTip == "Melee" then
        -- ✅ Đảm bảo không trùng connection
        if ScriptStorage.Connections.Melees then
            pcall(function() ScriptStorage.Connections.Melees:Disconnect() end)
        end

        ScriptStorage.CurrentMeleeData.Name = Child.Name

        -- Retry tối đa 5 lần để chờ Level property load
        task.spawn(function()
            for i = 1, 5 do
                if Child:FindFirstChild("Level") then
                    ScriptStorage.Melees[Child.Name] = Child.Level.Value

                    ScriptStorage.Connections.Melees = Child.Level.Changed:Connect(function(val)
                        ScriptStorage.Melees[Child.Name] = val
                        RefreshMelees()
                    end)

                    RefreshMelees()
                    -- Auto check skill đặc biệt
                    pcall(function() CheckMeleeBurstMove(Child) end)
                    return
                end
                task.wait(0.5)
            end
            warn("[MeleeCheck] Không tìm thấy Level trong " .. Child.Name)
        end)

    elseif string.find(Child.Name, "Fruit") then
        task.spawn(function()
            local fruitName = Child:GetAttribute("OriginalName")
            if fruitName and not table.find(ScriptStorage.IgnoreStoreFruits, fruitName) then
                Remotes.CommF_:InvokeServer("StoreFruit", fruitName, Child)
            end
        end)
    end
end
...
-- 🔄 Đăng ký kết nối cho LocalPlayer
function RegisterLocalPlayerEventsConnection()
    -- Auto bật Buso sau 6 giây
    task.spawn(function()
        task.wait(6)
    ...
    -- Clear connections cũ
    for _, conn in pairs(ScriptStorage.Connections.LocalPlayer) do
    ...
    ScriptStorage.Connections.LocalPlayer.HealthCheck = humanoid:GetPropertyChangedSignal("Health"):Connect(function()
    ...
    ScriptStorage.Connections.LocalPlayer.PointConnection = pointsInstance:GetPropertyChangedSignal("Value"):Connect(function()
        local current = pointsInstance.Value
        if OldPointValue ~= current then
            OldPointValue = current
            AddPoint()
        end
    end)
end

-- 🔁 Re-register khi Character respawn
...
print("[Melee System] ✅ Loaded successfully")
...
FunctionsHandler.Pole:RegisterMethod("Refresh", function()
    
    if ScriptStorage.Backpack["Pole (1st Form)"] then 
        
        return
    end

    -- Nếu level chưa đủ
    if ScriptStorage.PlayerData.Level < 500 then 
        return
    end 

    -- Nếu không ở First Sea
    if SeaIndex ~= 1 then 
        Remotes.CommF_:InvokeServer("TravelMain")
        return
    end

    -- Nếu boss Thunder God đang tồn tại → return 1 (tấn công)
    if ScriptStorage.Enemies["Thunder God"] then
        return 1
    else
        return 2 -- boss chưa có, hop server
    end
end)


FunctionsHandler.Pole:RegisterMethod("Start", function(state)
    if state == 1 then 
        SetTask("MainTask", "Item Pole | Attack Thunder God")
        CombatController.Attack("Thunder God")
    elseif state == 2 then
        SetTask("MainTask", "Item Pole | Find Server Thunder God")
        task.delay(1, function()
            Hop("Thunder God find")
        end)

    else
         SetTask("MainTask", "Item Pole | Completed")
    end
end)
