...
function Report(Message) 
    ...
    DiscordUrlTextLabel.Font = Enum.Font.Code
    ...
    local DiscordStroke = Instance.new("UIStroke", DiscordUrlTextLabel)
    DiscordStroke.Color = Color3.fromRGB(255, 255, 255)

    local DiscordGradient = Instance.new("UIGradient", DiscordStroke)
    ...
    local Under = Instance.new("TextLabel", Main)
    ...
    local UnderGradient = Instance.new("UIGradient", Under)
    UnderGradient.Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0, Color3.fromRGB(153, 102, 204)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(223, 115, 255))
    }

    -- Resize DropShadowHolder theo text
    ...
    getgenv().ContentSet = function(Content1, Content2, Content3)
        Content1 = Content1 or 'None (Not Found)'
        Content2 = (Content2 ~= '' and Content2) or 'Farming Level (or None)'
...
-- ====== FIXED RefreshMelees + Auto Refresh ======
...
-- Hàm refresh tất cả melee từ inventory
local function RefreshAllMelees()
    local success, Inventory = pcall(function()
        return Remotes.CommF_:InvokeServer("getInventory")
    end)
    if success and Inventory then
        ScriptStorage.Melees = {} -- reset trước khi ghi lại
        for _, item in ipairs(Inventory) do
            if item.Type == "Melee" then
                ScriptStorage.Melees[item.Name] = item.Mastery or 0
            end
        end
        RefreshMelees()
    end
...
-- Gọi sau khi load nhân vật / reconnect
task.spawn(function()
    repeat task.wait(3) until game.Players.LocalPlayer and game.Players.LocalPlayer:FindFirstChild("Data")
    RefreshAllMelees()
end)
...
-- Kiểm tra melee đang có trong character
...
-- Tự động refresh tất cả melee từ inventory để tránh thiếu
RefreshAllMelees()

-- Nếu cần, bạn vẫn có thể gọi RefreshPlayerData()
RefreshPlayerData()
...
function TweenController.Create(Position)
   ...
   if CaculateDistance(Position) > 2000 then
    GetPortal(Position)
      end 
...
FunctionsHandler.LevelFarm:RegisterMethod("Start", function(Level) 
  ...
  if Config.Settings.FarmBonestogetx2Exp and PlayerLevel >= 2025 and (getsenv(game.ReplicatedStorage.GuideModule)._G.ServerData.ExpBoost == 0 or PlayerLevel == MaxLevel ) and ( ScriptStorage.Backpack.Bones or { Count = 0 } ).Count < 500 then 
...
-- ==============================
-- AUTO MUA MELEE TỐI ƯU
-- ==============================

local MeleeLastCursor = 1
local MaxCursor = #MeleesTable

FunctionsHandler.MeleesController:RegisterMethod("Start", function()
    while true do
        if MeleeLastCursor > MaxCursor then
            MeleeLastCursor = 1
            task.wait(0.1)
        end
...
if not Config.Items.AutoFullyMelees then break end
        ...
        local Cursor = MeleeLastCursor
        local Melee = MeleesTable[Cursor]
        ...
        if not Data then
            print("[Debug] No data for", Melee)
            MeleeLastCursor = Cursor + 1
            continue
        end

        -- ===== Dragon Talon Special Check =====
        if Melee == "Dragon Talon" then
            if IsFireEssenceGave == nil then
                local success, result = pcall(function()
                    return Remotes.CommF_:InvokeServer("BuyDragonTalon", true)
                end)
                IsFireEssenceGave = success and typeof(result) ~= "string"
                if IsFireEssenceGave then
                    alert("Dragon Talon Purchased", tostring(result))
                end
            end
        ...
        if Melee == "Godhuman" and not GodHumanFlag then
            if (ScriptStorage.Melees["Dragon Talon"] or 0) > 399 then
                if not ScriptStorage.Melees.Godhuman then
                    ...
                    -- cập nhật flag tạm thời, vòng lặp sẽ tiếp tục check lại
                    GodHumanFlag = true
                    MeleeLastCursor = Cursor + 1
                    continue
                end
        ...
        -- ===== Kiểm tra điều kiện level & tiền =====
        local NextLevelReq = Data.NextLevelRequirement or 0
        local CurrentLevel = ScriptStorage.Melees[Melee] or 0
        if CurrentLevel < NextLevelReq then
            local PlayerData = ScriptStorage.PlayerData or {}
            ...
            -- Nếu đủ tiền và requirements → mua
            if HasEnough and Data.Requirements() then
...
local FireEssencePassed = Remotes.CommF_:InvokeServer("BuyDragonTalon", true)
    ...
    elseif Type == "Previous Hero" then 
     ...
     elseif Type == "Find Fire Essence" then
                ...
                CombatController.Attack({
                    ...
                    FunctionsHandler.LocalPlayerController.Methods.EquipTool:Call(ScriptStorage.ForceToUseSword and "Melee")
...
--[[SetTask("MainTask", "n/a")
    ...
    if not _G.EnableNearbyHop then
        return
    end 
